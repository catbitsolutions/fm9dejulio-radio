...
const COLORS = {
  ...
  light: { primary: '#DAA520', bright: '#FFD700', accent: '#C9A227', bg: '#f5f5f5', text: '#1a1a1a', muted: '#666666' }
}
...
const Icons = {
  ...
  twitter: <svg viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>,
  telegram: <svg viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/></svg>,
  email: <svg viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>,
  copy: <svg viewBox="0 0 24 24" fill="white" width="24" height="24"><path d="M16 1H4c-1.1 0-2 .9-2 2v14h2V3h12V1zm3 4H8c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h11c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm0 16H8V7h11v14z"/></svg>,
}

const SHARE_OPTIONS = [
  { name: 'WhatsApp', icon: Icons.whatsapp, color: '#25D366', url: (u: string, t: string) => `https://wa.me/?text=${encodeURIComponent(t + ' ' + u)}` },
  { name: 'Facebook', icon: Icons.facebook, color: '#1877F2', url: (u: string) => `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(u)}` },
  { name: 'Instagram', icon: Icons.instagram, color: '#E4405F', url: (u: string, t: string) => `https://www.instagram.com/create/story/?caption=${encodeURIComponent(t)}` },
  { name: 'X', icon: Icons.twitter, color: '#000000', url: (u: string, t: string) => `https://twitter.com/intent/tweet?text=${encodeURIComponent(t)}&url=${encodeURIComponent(u)}` },
  { name: 'Telegram', icon: Icons.telegram, color: '#0088cc', url: (u: string, t: string) => `https://t.me/share/url?url=${encodeURIComponent(u)}&text=${encodeURIComponent(t)}` },
  { name: 'Email', icon: Icons.email, color: '#EA4335', url: (u: string, t: string) => `mailto:?subject=${encodeURIComponent('FM 9 de Julio')}&body=${encodeURIComponent(t + '\n' + u)}` },
  { name: 'Copiar', icon: Icons.copy, color: '#6B7280', action: 'copy' },
]
...
export default function Home() {
  ...
  const colors = dark ? { ...COLORS.dark, primary: theme.primary, bright: theme.accent, bg: theme.bg } : COLORS.light
  ...
  return (
    <div style={{ minHeight: '100dvh', width: '100%', background: dark ? `linear-gradient(180deg, ${colors.bg} 0%, #1a1a1a 50%, ${colors.bg} 100%)` : colors.bg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'space-between', padding: '12px 10px', fontFamily: 'system-ui, sans-serif', boxSizing: 'border-box', position: 'fixed', inset: 0, overflow: 'hidden', color: colors.text }}>
      ...
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', flex: '0 0 auto', paddingTop: installPrompt ? '55px' : '0' }}>
        <div style={{ width: '100%', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
          ...
          <div style={{ display: 'flex', gap: '5px' }}>
            ...
            <div style={{ position: 'relative' }}>
              <button onClick={() => setShowTimer(t => !t)} style={{ background: sleepTimer ? colors.primary : 'transparent', color: sleepTimer ? colors.bg : colors.muted, border: `1px solid ${colors.primary}`, borderRadius: '6px', padding: '4px 8px', fontSize: '10px', cursor: 'pointer' }}>⏰ {sleepDisplay || 'Timer'}</button>
              {showTimer && (
        ...
        <p style={{ fontSize: '11px', color: colors.muted, margin: '2px 0 0 0' }}>Tres Isletas, Chaco</p>
      </div>
      ...
      {showStats && (
        <div style={{ width: '100%', maxWidth: '320px', background: 'rgba(0,0,0,0.3)', borderRadius: '10px', padding: '10px', marginBottom: '5px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
            <div style={{ textAlign: 'center', flex: 1 }}><div style={{ fontSize: '18px', fontWeight: 'bold', color: colors.primary }}>{stats.todayMinutes} min</div><div style={{ fontSize: '9px', color: colors.muted }}>Hoy</div></div>
            <div style={{ textAlign: 'center', flex: 1 }}><div style={{ fontSize: '18px', fontWeight: 'bold', color: colors.primary }}>{Math.floor(stats.totalMinutes / 60)}h {stats.totalMinutes % 60}m</div><div style={{ fontSize: '9px', color: colors.muted }}>Total</div></div>
            <div style={{ textAlign: 'center', flex: 1 }}><div style={{ fontSize: '18px', fontWeight: 'bold', color: colors.primary }}>🔥 {stats.streak}</div><div style={{ fontSize: '9px', color: colors.muted }}>Racha</div></div>
          </div>
      ...
      <div onClick={cycleMode} style={{ width: '100%', maxWidth: '320px', height: '50px', borderRadius: '8px', overflow: 'hidden', background: dark ? 'rgba(0,0,0,0.3)' : 'rgba(0,0,0,0.05)', border: `1px solid ${colors.primary}40`, cursor: 'pointer', position: 'relative' }}>
        ...
        <div style={{ position: 'absolute', bottom: '2px', right: '5px', background: 'rgba(0,0,0,0.6)', padding: '2px 6px', borderRadius: '6px', fontSize: '8px', color: colors.muted, display: 'flex', alignItems: 'center', gap: '3px' }}><span>{VISUALIZER_MODES[mode].icon}</span><span>{VISUALIZER_MODES[mode].name}</span></div>
      ...
      <div style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', flex: '0 0 auto' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '5px', padding: '4px 10px', borderRadius: '12px', background: isPlaying ? 'rgba(218, 165, 32, 0.25)' : 'rgba(255,255,255,0.1)', border: `1px solid ${isPlaying ? colors.primary : 'rgba(255,255,255,0.2)'}`, marginBottom: '12px' }}>
          <div style={{ width: '6px', height: '6px', borderRadius: '50%', background: isPlaying ? '#4ade80' : colors.muted, animation: isPlaying ? 'blink 1s infinite' : 'none' }} />
      ...
      <div style={{ width: '100%', display: 'flex', justifyContent: 'center', alignItems: 'flex-start', gap: '12px', flex: '0 0 auto', padding: '5px 0' }}>
        <button onClick={() => window.open(`https://wa.me/543644503323?text=${encodeURIComponent('🎤 Quiero enviar un saludo por FM 9 de Julio!\n\n')}`, '_blank')} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', padding: '0', width: '52px' }}>
          ...
          <span style={{ color: colors.muted, fontSize: '9px' }}>Saludo</span>
        </button>
        <a href="https://wa.me/543644503323" target="_blank" rel="noopener noreferrer" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', textDecoration: 'none', width: '52px' }}>
          ...
          <span style={{ color: colors.muted, fontSize: '9px' }}>WhatsApp</span>
        </a>
        <a href="https://www.facebook.com/fm9dejuliotresisletas" target="_blank" rel="noopener noreferrer" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', textDecoration: 'none', width: '52px' }}>
          ...
          <span style={{ color: colors.muted, fontSize: '9px' }}>Facebook</span>
        </a>
        <button onClick={() => setShowShare(true)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', background: 'none', border: 'none', cursor: 'pointer', padding: '0', width: '52px' }}>
          ...
          <span style={{ color: colors.muted, fontSize: '9px' }}>Compartir</span>
        </button>
        <a href="https://play.google.com/store/apps/details?id=com.radioshd.fm9dejulio" target="_blank" rel="noopener noreferrer" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '4px', textDecoration: 'none', width: '52px' }}>
          ...
          <span style={{ color: colors.muted, fontSize: '9px' }}>App</span>
        </a>
      </div>
      ...
      <div style={{ textAlign: 'center', flex: '0 0 auto', paddingBottom: 'env(safe-area-inset-bottom, 5px)' }}>
        ...
        <span style={{ color: colors.muted, fontSize: '8px' }}>Diseñado por <a href="https://wa.me/543644536161" target="_blank" rel="noopener noreferrer" style={{ color: colors.primary, textDecoration: 'none' }}>Davincho</a></span>
      </div>
          ...
          <div style={{ background: colors.bg, borderRadius: '20px 20px 0 0', padding: '20px', width: '100%', maxWidth: '400px', paddingBottom: 'calc(20px + env(safe-area-inset-bottom))' }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }}>
              {SHARE_OPTIONS.map(opt => (
                <button key={opt.name} onClick={() => opt.action === 'copy' ? copyUrl() : openShare(opt.url(RADIO_URL, SHARE_TEXT))} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px', background: 'none', border: 'none', cursor: 'pointer', padding: '8px' }}>
                  <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: opt.color, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{opt.icon}</div>
                  <span style={{ color: colors.muted, fontSize: '10px' }}>{opt.name}</span>
                </button>
              ))}
            </div>
      ...
      {showFs && (
        <div style={{ position: 'fixed', inset: 0, background: '#000', zIndex: 1000, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'absolute', top: '15px', left: 0, right: 0, textAlign: 'center', color: colors.text, textShadow: '0 2px 10px rgba(0,0,0,0.5)' }}>
            ...
            <p style={{ fontSize: '12px', margin: '3px 0', color: colors.muted }}>102.3 MHz - Tres Isletas, Chaco</p>
          </div>
      ...
      <style jsx global>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input[type="range"]::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 16px; height: 16px; border-radius: 50%; background: ${colors.primary}; cursor: pointer; box-shadow: 0 0 6px ${colors.primary}80; border: 2px solid ${colors.bright}; }
        input[type="range"]::-moz-range-thumb { width: 16px; height: 16px; border-radius: 50%; background: ${colors.primary}; cursor: pointer; box-shadow: 0 0 6px ${colors.primary}80; border: 2px solid ${colors.bright}; }
      `}</style>
    </div>
  )
}
